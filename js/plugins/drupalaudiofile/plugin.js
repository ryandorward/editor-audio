...
(function ($, Drupal, drupalSettings, CKEDITOR) {
  ...
  CKEDITOR.plugins.add('drupalaudiofile', {
    hidpi: true,
    init: function (editor) {
      // Add the commands for file and unfile.
      editor.addCommand('drupalaudiofile', {
        allowedContent: {
          a: {
            attributes: {
              '!href': true,
        ...
        exec: function (editor) {
          ...
          var saveCallback = function (returnValues) {
            ...
            // if (returnValues.attributes.href) { // for now only allow create, no update. @todo uncomment the above to work on update
              ...
              // @todo: make this non-kludgey
              returnValues.attributes['src'] = returnValues.attributes['href'];
              delete returnValues.attributes['href'];
              returnValues.attributes['controls'] = 'controls';
              // end kludge

              var style = new CKEDITOR.style({element: 'audio', attributes: returnValues.attributes});
          ...
          var dialogSettings = {
            title: fileElement ? editor.config.drupalAudioFile_dialogTitleEdit : editor.config.drupalAudioFile_dialogTitleAdd,
            dialogClass: 'editor-file-dialog'
          };

          // Open the dialog for the edit form.
          Drupal.ckeditor.openDialog(editor, Drupal.url('editor_audio/dialog/audio_file/' + editor.config.drupal.format), existingValues, saveCallback, dialogSettings);
        }
      });
      ...
      if (editor.ui.addButton) {
        ...
        editor.ui.addButton('DrupalAudioFile', {
          label: Drupal.t('Audio'),
          command: 'drupalaudiofile',
          icon: this.path + '/icons/hidpi/audio32.png'
        });
      }
      ...
      editor.on('doubleclick', function (evt) {
        ...
        if (!element.isReadOnly()) {
          if (element.is('audio') && element.getAttribute('data-entity-type') === 'file') {
            editor.getSelection().selectElement(element);
            editor.getCommand('drupalaudiofile').exec();
          }
        }
      });
  ...
  function getSelectedFile(editor) {
    ...
    if (selectedElement && selectedElement.is('audio')) {
    ...
    if (range) {
      ...
      return editor.elementPath(range.getCommonAncestor()).contains('audio', 1);
    }
    return null;
  }
